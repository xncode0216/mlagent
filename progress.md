...
## 2026-07-29 M5 第一片：目标列在计划阶段可选

- **动机**：推断只是启发式（名称提示 + 基数 + 末列位置），猜错时用户此前**没有任何纠正手段**——「修订计划」只是拒绝这一版让 agent 重出，参数不变、结果相同。
- **修回了一处我自己引入的回归**：训练卡片上原本就有目标列选择器（`modelCards.ts:46`），它靠"改偏好 → 下一条消息带上 → 后端回显到 `training_config.props`"这条回路生效。上一片让计划优先于 context 之后，**那条回路断了**——选择器还在，但选了没用。这一片把选择移到计划阶段，选择经由计划本身生效。
- **为什么纠正必须落在计划上而不是绕过计划**：目标列决定了哪些列进 `drop_columns`、哪些进 `feature_columns`、`pipeline_script` 怎么写。在计划之外改一个字段会让计划与它的派生产物自相矛盾——训练脚本本来就会以 `Preprocessing plan target column does not match the requested target` 拒绝。所以后端接口收 `target_column` 后是**重算整份计划**，不是改一个键。
- **不在数据集里就 400**：放行只会把失败推迟到训练时，那里的报错离用户更远。
- **选择器只在审批检查点出现**，变换执行后不再提供：那时计划已经产出数据集，改它不会回头重算。
- **控件 id 用 `plan_target_column` 与训练卡片的 `target_column` 区分**。两者后果不同（重算整份计划 vs 切换本次训练的目标列），共用 id 会让计划卡片上的选择静默走成训练卡片的行为。这条有测试固定，并做了**负向验证**：把 id 改回去后新增用例中的 2 项确实失败。
- **重算时刻意不带特征选择**：那是上一版计划里的选择，换目标列后未必仍然成立（原目标列会变成一个普通特征）。
...
## 2026-07-29 M5 第三片：批准之前就能看清变换会做什么

- **缺口不在"没有列对照"，而在"看清楚必须先批准"**。变换报告与 `TransformDiffPreview` 列对照视图早就有了，但它们全都在**执行之后**才拿得到；而批准会写出变换后的数据集，是一步不可逆的动作。审批检查点的意义本来就是"批准前先看清楚"，此前恰恰做不到。
- **预览与执行必须共用同一段计算**，所以先把 `_apply_preprocessing_plan` 从执行函数里拆出来。分成两份实现的话预览就会开始说谎——而用户是**照着预览做的批准决定**，那时说谎比没有预览更糟。这一条有逐字段比对的测试固定。
- **预览刻意不带 `output_dataset_path`**：它没有产出数据集，写上就是谎报。
- **前端一行渲染代码都没改**：右侧面板的 `isTransformationReport` 是**按形状**判定的（`input_shape`/`output_shape`/`transformations`/`encoded_feature_columns` 四个键齐备即可），预览摘要与执行摘要同形状，列对照视图直接就能渲染它。这一点是先去读了判定条件才确定的，不是碰运气。
- **顺带修掉上一片留下的脏活**：管道脚本此前无条件 `import MinMaxScaler`，选 none 或 standard 时那是个用不上的导入。是 ruff 扫到 `.tmp_pytest` 里遗留的生成脚本才发现的——脚本是交给用户的产物，留个没用的 import 是脏活。
- **负向验证两处**：让预览另算一套（`drop_columns` 置空）→ 逐字段比对用例失败；移除卡片上的预览动作 → 对应用例失败。
- **一次操作失误**：负向验证后用 `git checkout --` 还原文件，把同一文件里**未提交的**改动一起冲掉了（HEAD 没有那段代码）。全量测试立刻报出来，重新加回。教训：负向验证要改的那个文件如果同时含未提交改动，只能改回去，不能用 checkout。
- **门禁**：后端 `264 passed, 3 skipped` + ruff；前端 `43 files / 350 tests`、tsc、ESLint、build + 预算门禁（gzip 136.21kB）、Playwright `10 passed`。
- **M5 至此的完整形态**：目标列、特征列、数值填充、缩放、类别填充都可在审批检查点调整，每一项都会重算整份计划与管道脚本；调完可以先预览再决定批准。剩下 ordinal 编码未做（会改输出列形状并牵动训练，另开片）。
