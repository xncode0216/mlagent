...
- [ ] **P1-1 Frontend state architecture** (L). Evidence: `zustand` and `@tanstack/react-query` are dependencies but used 0× in `src/`; all state lives in `AppShell.tsx` (1844 lines) via ~100 `useState` + 30+ props drilled to `AgentWorkspace`. Fix: adopt react-query for server data (cache/invalidate/retry/optimistic) and zustand for global UI state; decompose `AppShell` and lift data fetching out of components.
  - **进行中（2026-06-14，地基 + 首迁，增量行为保持）**：① 安全网——补 `AppShell.smoke.test.tsx`（@testing-library/react + jsdom，importOriginal 桩 api 层）+ `src/test/{renderWithProviders.tsx,websocketStub.ts}`，重构前先建 render 回归网；② 地基——`src/lib/queryClient.ts`（保守默认）+ `App.tsx` 接 `QueryClientProvider`；③ 首迁——只读的 evolution `protocols` 迁到 `useEvolutionProtocolsQuery`（零 mutation、最隔离）。三切片各自 `vitest`(16 文件/93 passed) + `eslint` + `npm run build`(tsc+vite) 全绿，分别本地提交。**剩余（后续会话）**：迁 GPU 轮询 + `lessons`/`injectionLogs`/`trainingRuns`（各有 5~7 处 setState 纠缠，需逐个改 `invalidateQueries`）→ 再迁 sessions/messages/events/task-states 与 bootstrap+文件树（`useMutation`）→ 落地 `app/uiStore.ts`（zustand）逐字段消除 props drilling → 拆 `AppShell` 为薄容器。
