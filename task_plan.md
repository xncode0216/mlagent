...
- [ ] **P1-6 Decompose backend orchestrator** (M). Evidence: `agent_orchestrator_service.py` = 4058 lines (43% of backend app). Fix: split into intent classification / workflow orchestration / per-stage executors / artifact assembly modules. → **进行中 2026-06-13（`feat/p0-backend-hardening`，增量、行为保持）**：新建包 `app/services/agent_orchestrator/`，原文件保留为 facade（维持公共 import 路径，`ws.py` 与 3 个 import 该模块的测试无需改动）。已落地 3 个切片（每个切片后 `pytest` 仍 169 passed / 3 skipped、`ruff` 全绿）：**切片 1** 外移纯模块级代码——`contexts.py`（7 个 dataclass）、`support.py`（`_utc_now`/`_relative_path`/`_resolve_active_file` + 常量）、`artifacts.py`（审批持久化 + artifact 写入 + `_render_transformation_report`）、`tools.py`（`_default_llm_client` + 只读分析工具注册表）、`intent.py`（`classify_intent`，原 276 行 `_classify_intent` 体，方法改为委托）；**切片 2** `commands.py`——9 个 `agent_command`/props 事件构建器（7 个有状态的以 orchestrator 作 `meta` 参数读取 `trace_id`/`session_id`，facade 方法 `return f(self, ...)` 委托）；**切片 3** `runs.py`——8 个纯 run/dataset/context 查询 helper（`_match_run_by_active_file`/`_candidate_dataset_summaries` 等，方法委托）。facade 由 4263 → 3229 行（累计 −1034，约 −24%），新包 8 个内聚子模块共 ~1248 行。_剩余_：切片 4（15 个 `_run_*` stage runner 经 `StageRunnersMixin` 外移）、切片 5（流式/消息 + 事件 helper 经 `MessagingMixin` 外移），使 `AgentOrchestrator` 收敛为薄分发器。
